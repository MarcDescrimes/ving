...
Visualization requires at least one bam file and coordinates of a genome region 
to visualize: chromosome name, start and end.
    Rscript ving.R [options] <input> [<input2> <input3> ...]
Therefore, the minimal command line is, for example :
    Rscript ving.R -c chr15 -S 210400 -E 224400 WT1.bam
    
This produces an "output.png" image with the default parameters.

Another example with 4 bam files :
...
If you want to pool normalized data, you have to set a normalization coefficient for each bam files :
...
-n/--normalization-coefficient <number,number,...>
    ...
    To perform a Fragment Per Million mapped read (FPM) normalization, set for each bam file
    a factor equal to (10^6)/<number of mapped fragments>
...
-L/--smoothLength <number>
    Sets the length of the sliding window smoothing. The default is NA and allows ving to compute
    an optimal length, according to the length of the requested display region.
